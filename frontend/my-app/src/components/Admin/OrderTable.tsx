...
const OrderTable = () => {
  ...
  const updateReturn = async (orderId: string, status: string) => {
    if (!confirm(`Are you sure to ${status} this return?`)) return;

    try {
      const token = localStorage.getItem("token");

      await axios.put(
        `http://localhost:1000/api/v1/update-return-status/${orderId}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${token}`
    ...
    <div className="flex min-h-screen bg-zinc-900 text-white">
      ...
      <div className="flex-1 p-6">
        ...
        {filteredOrders.map((order: any) => {
          ...
          // const isReturnFlow = order.returnStatus === "requested";
          return (
            ...
            <div key={order._id} className="bg-zinc-800 p-4 mb-3 rounded">
              ...
              <select
                ...
                // disabled={isReturnFlow}
                // value={order.status}
                // onChange={(e) => updateStatus(order._id, e.target.value)}
                // className={`mt-3 px-3 py-1 rounded text-white font-semibold w-auto ${isReturnFlow ? "opacity-50 cursor-not-allowed" : ""
                //   } ${statusColor[order.status] || "bg-zinc-700"}`}
