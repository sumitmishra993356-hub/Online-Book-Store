...
const ViewBookDetails = () => {
  ...
  const deleteBook = async () => {
    ...
    try {
      ...
      await axios.delete(
        `http://localhost:1000/api/v1/delete-book/${book?._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  `}>
          ...
          {role === "admin" && (
            <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <button
                onClick={editBook}
                className="group flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-2.5 rounded-xl
      bg-linear-to-r from-yellow-500/10 to-yellow-600/10
      ...
      transition-all duration-300"
              ...
              <button
                onClick={deleteBook}
                className="group flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-2.5 rounded-xl
      bg-linear-to-r from-red-500/10 to-red-600/10
